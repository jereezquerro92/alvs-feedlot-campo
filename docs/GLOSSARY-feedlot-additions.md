...
| model field (origin ref) | `source_kind`, `source_id` | generic link from a `LedgerEntry`/`FeedStockMovement` to its originating event ([[adr-49-domain-layer-and-growth-by-addition]], [[adr-25-account-ledger]]) | a per-domain FK on `LedgerEntry`; `content_type`/`object_id` framing if a plain pair suffices |
...
| model (expense event) | `ExpenseEvent` | an immutable `CostedEvent` that posts a `service` debit via the ledger `(source_kind, source_id)` seam — gasto extra a un cliente ([[adr-44-field-operational-roles]] decisión 6, [[adr-49-domain-layer-and-growth-by-addition]] regla 4) | `Charge`, `Expense`, `Debt`, `Gasto`, a manual `LedgerEntry` |
...
| architecture term (spine) | `spine` | the shared apps every domain rides on — `clients`, `ledger`, `assets`, `market`, `fx` — reused by a new domain rather than duplicated ([[FEEDLOT]], [[adr-49-domain-layer-and-growth-by-addition]], [[adr-32-multi-rubro-assets]]) | `core`, `base`, `foundation`, `kernel`, `common` |
