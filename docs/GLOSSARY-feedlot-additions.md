...
| assistant endpoint segment (ask) | `ask` — `/api/assistant/ask/` | the page-context surface of the `assistant` app: one utterance plus the current page path in, grounded prose plus permission-filtered links out ([[API]], [[adr-55-page-context-assistant]]). Distinct from the router's `route` segment, which chooses from a closed enum and generates nothing — the two are different acts, each forbidding the other's name | `query`, `prompt`, `interpret`, `route` (that is the router's segment), `chat` |
| model (assistant audit) | `AssistantQuery` | one row per page-context ask: utterance, page, answer, raw model output, links, status, model id, latency, user ([[adr-55-page-context-assistant]] rule 6). Retired only by `purge_assistant_audit` | `IntentQuery` (that is the router's audit row), `AskLog`, `AssistantLog`, `Query` |
| permission class (page-context assistant) | `CanUseAssistant` (`backend/apps/assistant/permissions.py`) | DRF permission gating `AskView`: authenticated AND in the granting Django group ([[adr-55-page-context-assistant]] rule 7, [[API]]) | `AssistantAccess` (that gates the conversational `/api/conversations/…` surface), `CanUseRouter`, `IsAssistantUser` |
| model field (chat drawer opt-in) | `chat_drawer_enabled` | per-user boolean, default `false`, mounting the page-context assistant's drawer edge tab ([[API]], [[adr-55-page-context-assistant]] rule 7). A preference, never authority — it grants nothing on its own | `chat_enabled`, `assistant_enabled`, `show_chat`, treating it as a permission |
| component (assistant drawer) | `AssistantDrawer` | the drawer hosting the page-context **generating** tier; sibling of `ChatDrawer`, which hosts the **router** (choosing) tier. Two components, never one with a mode flag, so [[adr-15-chatbot-two-tier]] rule 1's disjunction stays visible in the file tree ([[COMPONENTIZATION]]) | reusing `ChatDrawer` for both tiers, `ChatDrawer` with a `tier`/`mode` prop, `GenerativeDrawer` |
...
| permission class (assistant access) | `AssistantAccess` | gates the conversational asesor (`/api/conversations/…`); staff read/ask, `lot_owners` read/ask ONLY their bound client — the third client-keyed portal surface ([[adr-45-lot-owner-assistant-access]], [[adr-44-field-operational-roles]]) | `ChatPermission`, `ConversationPermission`, folding it into `ClientScopedReadPermission` |
| permission class (expenses access) | `ExpensesAccess` | gates the extra-expenses surface (`/api/expenses/`); field managers, workshop, feedlot owners ([[adr-44-field-operational-roles]]) | `ExpensePermission`, `GastosAccess`, `ChargeAccess` |
| Django app (expenses) | `expenses` | extra-charges app — labor/fuel/machinery billed to a client's account (Fase 4d, [[adr-44-field-operational-roles]] decisión 6) | `charges`, `debts`, `gastos`, `deudas`, `ledger` (that is the account app) |
| model (expense event) | `ExpenseEvent` | an immutable `CostedEvent` that posts a `service` debit via the ledger `(source_kind, source_id)` seam — gasto extra a un cliente ([[adr-44-field-operational-roles]] decisión 6, [[adr-24-feedlot-domain]] regla 4) | `Charge`, `Expense`, `Debt`, `Gasto`, a manual `LedgerEntry` |
| ledger origin ref value (Fase 4d) | `source_kind` = `expense_event` | the `LedgerEntry` back-link for an `ExpenseEvent` debit; reuses `Concept.SERVICE`, no new `Concept` ([[adr-44-field-operational-roles]], [[adr-25-account-ledger]]) | a per-domain FK, a new `Concept` value, `expense`/`gasto` |
| Django app (breeding) | `breeding` | cattle reproduction events app — cría/recría (Fase de cría, [[adr-46-breeding-reproduction]]) | `reproduction`, `cria`, `recria`, `gestation`, `livestock` (that is the cattle app) |
| model (service) | `Service` | an immutable reproductive-service event (natural/AI/IATF/embryo transfer) on an `Animal` XOR `Lot` ([[adr-46-breeding-reproduction]]) | `Insemination`, `Mating`, `Servicio`, `Breeding`, `Cover` |
| model field (service method) | `method` — `natural`\|`ai`\|`iatf`\|`embryo_transfer` | how the cow was serviced ([[adr-46-breeding-reproduction]]) | `type`, `kind`, `IA`, `TE`, Spanish values |
| model field (service price) | `service_price` | the insemination fee snapshotted on an AI/IATF service to a boarding client ([[adr-46-breeding-reproduction]], [[adr-25-account-ledger]]) | `price`, `fee`, `cost`, `tarifa`, Spanish values |
| model (pregnancy check) | `PregnancyCheck` | an immutable pregnancy-diagnosis event — tacto/diagnóstico de preñez; posts NO ledger entry ([[adr-46-breeding-reproduction]]) | `Palpation`, `Tacto`, `PregCheck`, `Diagnosis`, `Preñez` |
| model field (pregnancy check result) | `result` — `pregnant`\|`empty`\|`uncertain` | the diagnosis outcome ([[adr-46-breeding-reproduction]]) | `status`, `positive`, `preñada`/`vacia` as values, Spanish values |
| model field (pregnancy check method) | `method` — `palpation`\|`ultrasound`\|`blood` | how pregnancy was diagnosed ([[adr-46-breeding-reproduction]]) | `type`, `way`, `eco`, Spanish values |
| model (calving) | `Calving` | an immutable calving event — parición; a live individual calving creates a calf `Animal`; posts NO ledger entry ([[adr-46-breeding-reproduction]]) | `Birth`, `Parto`, `Paricion`, `Delivery`, `Freshening` |
| model field (calving outcome) | `outcome` — `live`\|`stillborn`\|`aborted` | the calving result ([[adr-46-breeding-reproduction]]) | `result`, `status`, `alive`, Spanish values |
| model field (calving ease) | `calving_ease` — `normal`\|`assisted`\|`caesarean` | the difficulty of the calving ([[adr-46-breeding-reproduction]]) | `difficulty`, `dystocia`, `ease`, Spanish values |
| model field (calf link) | `calf` (nullable FK on `Calving` → `livestock.Animal`) | the calf `Animal` a live individual calving created; genealogy is derived from it, never a `dam`/`sire` field on `Animal` ([[adr-46-breeding-reproduction]]) | adding `dam`/`sire` to `Animal`, `offspring`, `ternero` |
| model (weaning) | `Weaning` | an immutable weaning event — destete; the recría hand-off; posts NO ledger entry ([[adr-46-breeding-reproduction]]) | `Wean`, `Destete`, `Separation` |
| model field (weaning purpose) | `purpose` — `replacement`\|`sale`\|`undecided` | what the weaned animal is destined for — reposición/venta ([[adr-46-breeding-reproduction]]) | `destination`, `destino`, `fate`, Spanish values |
| model (IATF protocol) | `IatfProtocol` | a reusable editable IATF hormone-synchronization protocol template ([[adr-46-breeding-reproduction]]) | `Protocol`, `SyncPlan`, `Protocolo`, `IATFPlan` |
| model (IATF protocol step) | `IatfProtocolStep` | one step of an `IatfProtocol` at a relative `day_offset` ([[adr-46-breeding-reproduction]]) | `Step`, `ProtocolLine`, `Paso`, `ProtocolItem` |
| ledger origin ref value (breeding) | `source_kind` = `breeding_service` | the `LedgerEntry` back-link for an AI/IATF `Service` debit to a boarding client; reuses `Concept.SERVICE`, no new `Concept` ([[adr-46-breeding-reproduction]], [[adr-25-account-ledger]]) | a per-domain FK, a new `Concept` value, `service`/`ia` |
| Django app (genetics) | `genetics` | semen/embryo/DEP genetics app — genética (Fase de cría, [[adr-47-genetics-semen-embryo]]) | `semen`, `breeding` (that is the reproduction app), `genes`, `genetica` |
| model (sire) | `Sire` | an editable breeding bull catalog row — reproductor; may link an own `Animal` or be external ([[adr-47-genetics-semen-embryo]]) | `Bull`, `Toro`, `Reproductor`, `Father` |
| model field (sire registry) | `registry_id` | the external genetic-registry id of an external `Sire` ([[adr-47-genetics-semen-embryo]]) | `registry`, `code`, `hba`, Spanish values |
| model (breeding value) | `BreedingValue` | a DEP/EPD row for a `Sire`: `(trait, value, accuracy, source, date)` — editable catalog ([[adr-47-genetics-semen-embryo]]) | `EPD`, `DEP`, `GeneticValue`, `Dep`, `Merit` |
| model field (breeding value trait) | `trait` — `birth_weight`\|`weaning_weight`\|`milk`\|`ribeye_area`\|`marbling`\|`scrotal`\|`other` | the evaluated genetic trait ([[adr-47-genetics-semen-embryo]]) | `characteristic`, `caracteristica`, `feature`, Spanish values |
| model (semen batch) | `SemenBatch` | an editable straw-batch catalog row — partida de pajuelas; stock is Σin−Σout, never stored ([[adr-47-genetics-semen-embryo]]) | `SemenLot`, `StrawBatch`, `Pajuelas`, `SemenStock` |
| model (semen movement) | `SemenMovement` | an immutable in/out straw movement of a `SemenBatch`; stock derived ([[adr-47-genetics-semen-embryo]]) | `StrawMovement`, `Movement`, `Movimiento`, `FeedStockMovement` (that is feed) |
| model field (semen movement reason) | `reason` — `purchase`\|`collection`\|`insemination`\|`sale`\|`discard`\|`adjustment` | why the straws moved ([[adr-47-genetics-semen-embryo]]) | `type`, `motivo`, `cause`, Spanish values |
| model (semen sale) | `SemenSale` | an immutable semen-sale event; posts a `sale` credit to the own account + a `SemenMovement` out — venta de semen ([[adr-47-genetics-semen-embryo]], [[adr-43-sale-settlement]]) | `Sale` (bare — collides with `Exit` sale), `VentaSemen`, `StrawSale` |
| model field (semen sale buyer) | `buyer_name`, `buyer_client` (nullable FK) | the informational buyer of a semen sale; the credit posts to the own account regardless ([[adr-47-genetics-semen-embryo]]) | `client` (that would imply a client debit), `comprador`, `customer` |
| model (embryo batch) | `EmbryoBatch` | an editable embryo-batch catalog row; stock is Σin−Σout, never stored ([[adr-47-genetics-semen-embryo]]) | `EmbryoLot`, `EmbryoStock`, `Embriones` |
| model (embryo movement) | `EmbryoMovement` | an immutable in/out movement of an `EmbryoBatch`; stock derived ([[adr-47-genetics-semen-embryo]]) | `Movement`, `Movimiento`, `EmbryoStockMovement` |
| model (embryo flush) | `EmbryoFlush` | an immutable donor-collection event that produces embryo inventory — colecta/aspiración; posts NO ledger entry ([[adr-47-genetics-semen-embryo]]) | `Flush`, `Collection`, `Aspiration`, `Colecta`, `Recovery` |
