...
2. Recreating, renaming, or transferring a repo rotates its OIDC identity. Every trust entry naming that repo MUST be re-derived in the same batch as the change, and where the trust lives on a shared role, the mutation is recorded in [[INVENTORY]] under the discipline [[INFRASTRUCTURE]] owns.
