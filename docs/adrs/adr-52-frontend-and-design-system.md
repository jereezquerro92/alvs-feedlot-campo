---
title: adr-52-frontend-and-design-system
type: adr
category: frontend
...
3. The interactivity ladder is escalated in order, never skipping a rung: server-rendered HTML → [[HTMX]] → Svelte island. Escalation is a per-feature decision, and the HTMX-vs-Svelte criteria that decide it are owned by [[HTMX]].
...
6. Frontend tests follow [[FRONTEND]] (bun test runner, created per feature) and are excluded from [[TDD]].
...
- **NEVER** skip a rung of the ladder — a Svelte island for state a server render or HTMX could already own (rule 3). Escalation is a decision made per feature against [[HTMX]]'s criteria, not a default reach.
...
- [[docs/constitution/REQUIREMENTS]] — the bun/npm/Node pins
