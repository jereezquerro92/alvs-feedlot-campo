...
6. The local database is PostgreSQL 17. Local layer-2 cache is `DatabaseCache` in that database; cloud Valkey is not mirrored in Compose ([[adr-06-cache]] rule 2, [[CACHE]], [[DOCKER]]).
...
- **NEVER** add a Compose Redis/Valkey service as a silent second local cache strategy beside `DatabaseCache` without updating [[DOCKER]] and [[CACHE]] in the same change (rule 6). Local defaults to the database-backed layer 2.
...
- **No Redis in Compose because no cache server anywhere** — the FORBIDDEN entry and rule 6 wording until 2026-08-07 that forbade Redis in Compose as an echo of the absolute [[adr-06-cache]] prohibition. Cloud now runs ElastiCache Valkey; local keeps `DatabaseCache` without a Compose cache container. The absolute ban reopens only if [[adr-06-cache]] rule 1 is withdrawn.

## RELATED

### related adrs

- [[docs/adrs/adr-06-cache]] — Valkey in cloud; local `DatabaseCache` for layer 2
...
- [[docs/CACHE]] — local vs cloud layer 2
