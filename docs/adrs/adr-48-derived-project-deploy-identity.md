...
6. Provisioning is a separate act from this rule set, and its record is [[INVENTORY]]. Nothing in a document, workflow, or test may assume this project has provisioned resources while [[INVENTORY]] records none — the same born-dead discipline [[INFRASTRUCTURE]] imposes on any resource, applied here to the case of no resources at all. Any change to rules 1–3 is semantic and MUST supersede this ADR ([[adr-00-adr-doctrine]] rule 4).
