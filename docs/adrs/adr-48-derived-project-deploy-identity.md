...
4. Ownership divergence is recorded here and resolved elsewhere. The owning account of a given repository is a per-repository fact, and [[GH]] is the document that records it. This ADR does not amend [[adr-08-github-and-git]] rule 1, which names `kodexArg`: turning that rule from a constant into a per-repository lookup alters its force and is therefore a supersession, which only the owner may authorize ([[adr-00-adr-doctrine]] rule 4). Until that supersession is written, the divergence between adr-08 rule 1 and what [[GH]] records stands **open and disclosed**, never closed by a local exception. Everything else adr-08 rules — `main` is integration, `prod` is production, and only the owning `gh` identity pushes those lines ([[adr-19-issue-worktree-pr]] rule 3) — binds unchanged and is not in question.
