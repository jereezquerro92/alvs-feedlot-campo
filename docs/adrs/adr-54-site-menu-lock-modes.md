...
3. The menu is NEVER invisible: wherever the shell menu mounts, either the rail or the FancyDrawer peek caret is present and operable. A locked preference plus a CSS hide without an unlocked fallback is a defect. **The only condition on the menu is that the session has a role** ([[adr-20-authorization-lobby]] rule 1): no page, path, query parameter or selected entity is a precondition for the chrome. A client is per-link data — an unselected one scopes no link and suppresses nothing. Whether an individual *item* is shown is rule 9's question, separate from this one, and it never resolves to hiding the menu.
...
9. A menu **item** the session may not see is not rendered, and a section left with no visible items renders no heading. Which groups may see an item is not decided here and is never a second RBAC matrix: the menu mirrors the group names the matrix already grants ([[adr-44-field-operational-roles]] rule 1), and the mirror holds **group names only** — the same values `/api/me/` returns, so the frontend still knows nothing of how the server decides ([[adr-53-api-membrane]] rule 2). The item→permission-class binding, and the test that fails on drift, live on the **server** side: it is the only side permitted to know both halves, because the membrane's prohibition runs one way. Trimming is convenience, never the barrier (adr-44 rule 8): an unclassified item stays visible and the server refuses it. Hiding every item never hides the menu — rule 3 is unconditional, and an empty menu still presents its rail or peek caret.
...
- **NEVER** write the item→groups mapping as its own policy (rule 9). It mirrors the matrix under a server-side binding test; a hand-decided nav matrix is a second RBAC authority that will disagree with the one actually enforced.
- **NEVER** name a permission class, app path or any server internal in the menu's mapping or its frontend tests (rule 9, [[adr-53-api-membrane]] rule 2). Group names are contract data and may cross; the binding that knows both halves lives on the server side, tests included.
- **NEVER** hide an item the mapping does not classify (rule 9). An unclassified item stays visible and gets a backend refusal; disappearing silently is the harder bug to find.
- **NEVER** let item trimming stand in for authorization (rule 9, adr-44 rule 8). The route must refuse the request on its own, because a hidden link is still reachable by typing the URL.
...
- **A nav endpoint returning the session's allowed items** — the backend computing the visible menu and the frontend rendering whatever arrives. It is the strictly DRY-er shape and it lost on cost: it needs an [[API]] row, a serializer and a round trip to decide something purely cosmetic, when `/api/me/` already returns `groups`. Rule 9's mirror-plus-binding-test buys the same no-drift guarantee for a test file. It would reopen if item visibility ever depended on state the session groups cannot express — a per-client entitlement, a feature flag — because then the mirror would have nothing local to mirror.
- **A hand-decided nav permission table** — the fast version of rule 9: pick per item which roles look right. Rejected as the second RBAC matrix [[adr-44-field-operational-roles]] rule 1 exists to prevent; it would agree with the server on the day it was written and never again.
- **The drift check as a frontend test reading `roles.py`** — the first shape rule 9's binding was built in, and it worked. Rejected on the ADR guardian's `violation` verdict of 2026-08-06: [[adr-53-api-membrane]] rule 2 says *no frontend file* may name a permission class or an app path, and rule 5 makes the breach the knowledge itself rather than any wrong behaviour — a test file is no exemption. The same guarantee was moved to a server-side test that reads the frontend's group-name table, which crosses the membrane in the direction it permits. It would reopen only if adr-53 rule 2 gained an explicit carve-out, which was considered and not taken.
...
- [[adr-44-field-operational-roles]] — rules 1 and 8, the matrix rule 9 mirrors and why trimming is not the barrier
- [[adr-53-api-membrane]] — rule 2, why rule 9's binding test lives on the server side
