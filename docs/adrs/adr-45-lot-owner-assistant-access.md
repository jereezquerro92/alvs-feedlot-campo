...
1. Una sesión `lot_owners` alcanza `GET`/`POST` de `/api/conversations/…` ([[adr-35-conversational-assistant]]) únicamente sobre el cliente ligado a su `AccessRequest`. La lista que [[adr-44-field-operational-roles]] regla 3 enumera pasa a ser tres: métricas, cuenta y asesor. Ninguna otra ruta se abre.
...
- **Abrir el asesor a `lot_owners` sin ADR** — tratarlo como una lectura ya autorizada y agregar la ruta. Rechazado: [[adr-44-field-operational-roles]] regla 3 exige el vehículo, y sin él la lista enumerada deja de ser enumerada.
...
- [[docs/adrs/adr-44-field-operational-roles]] — reglas 1 y 3, la matriz y la lista de rutas que esto amplía
