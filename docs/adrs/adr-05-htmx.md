...
1. HTMX is in the stack, pinned in [[REQUIREMENTS]] like every other package ([[adr-02-initial-stack]] rule 1). It is rung 2 of the interactivity ladder ([[adr-04-frontend-and-design-system]] rule 3). Prefer it over a Svelte island whenever the state is server-owned; criteria in [[HTMX]].
...
3. Fragment routes are endpoints, and [[adr-03-api-and-backend]] rule 4 is where that is decided — this ADR adds nothing to it and states no second version of it.
4. Using HTMX is decided per feature against [[HTMX]]'s criteria — never "every page must be HTMX".
5. Reserved. The version pin is [[adr-02-initial-stack]] rule 1, cited from rule 1 above.
...
- **NEVER** wire an `hx-*` attribute at a route absent from [[API]] ([[adr-03-api-and-backend]] rule 4). A fragment route living only in a template is a shadow route, invisible to the contract.
...
- **Stating the fragment-route rule here** — rules 3 and 5 each carried their own version of a rule another ADR owns, until 2026-08-02: rule 3 restated [[adr-03-api-and-backend]] rule 4, and rule 5 restated [[adr-02-initial-stack]] rule 1. Both were dropped to pointers because two statements of one rule drift, and this ADR was the copy, not the owner ([[adr-00-adr-doctrine]] rule 1). Nothing about what the project requires changed.
...
- [[docs/adrs/adr-03-api-and-backend]] — rule 4, the owner of the fragment-route case rule 3 points at
- [[docs/adrs/adr-02-initial-stack]] — rule 1, the owner of the version pin
