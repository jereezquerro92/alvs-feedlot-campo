...
10. This is a capability layer, not a doctrine change. Cognito remains the sole authenticator ([[adr-10-auth]]), [[CACHE]] gains no cache server ([[adr-06-cache]]), every response carries an explicit `Cache-Control` and this one is `no-store`, the route enters [[API]] before code ([[adr-51-api-and-backend]] rule 1) and every variable it reads enters [[VARIABLES]] first (rule 7).
