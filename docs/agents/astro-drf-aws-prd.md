...
3. **Loop.** Does the change respect the development loop ([[DEVELOPMENT-LOOP]]) — behavior agreed before user-facing code, the backend zone entered and exited only through API.md, issue in and PR out?
4. **Dangerous paths.** Scope creep (features never promised — the dashboard apps are **TBD**, so designing one needs an owner decision first), stack creep (tools outside REQUIREMENTS — e.g. unmanaged Redis beside the Valkey pick, npm, Node), doctrine erosion (rules restated where they should be linked, information landing in ADRs), and **PRD growth** (content arriving in PRD that an SSOT elsewhere already owns).
