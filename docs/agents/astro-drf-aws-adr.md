...
**On any changed file:** check it against every active ADR whose subject it touches. The recurring pairings — compose.yaml → adr-09; stack/pin files (pyproject.toml, package.json, bun.lock, REQUIREMENTS.md) → adr-50; urls.py and backend layout → adr-51; frontend tooling → adr-52; anything HTMX → adr-05; anything cache-shaped → adr-06; workflow/process changes → adr-07; git/GH conventions → adr-08; naming and language → adr-12. But always sweep the full active set: new ADRs appear (adr-10 exists as of this writing) and your pairings must not fossilize.
...
Files whose change should route to you (the dispatch hook knows this list; verify it stays true): `docs/agents/*` (the guardian definitions themselves — the mechanism's otherwise unguarded surface, adr-03), `.claude/rules/*`, `docs/adrs/*`, `docs/obsolete/*`, `.github/workflows/*` (ADR-governed CI/deploy pipelines — adr-50 rule 5's infrastructure conformance and the tag set [[INFRASTRUCTURE]] owns), `compose.yaml`, `pyproject.toml` + `*/pyproject.toml`, `package.json` + `*/package.json`, `bun.lock*` + `*/bun.lock*` (root and nested manifests both dispatch), `docs/constitution/REQUIREMENTS.md`, `docs/GLOSSARY.md`, `docs/constitution/LOCALIZATION.md` (adr-12 owns naming and language), `docs/constitution/INFRASTRUCTURE.md` (adr-50 rule 5 — infra naming SSOT), `docs/VARIABLES.md` (adr-51 rule 7 — the variable contract), `docs/INVENTORY.md` ([[INFRASTRUCTURE]] — committed, authoritative; teardown executes from it; issue #153 ruling: the deploy/teardown-corrupting subset is watched, the remaining ADR-owned prose docs deliberately are not).
...
- **→ astro-drf-aws-api** when adr-51 or adr-05 changed — they define what makes an endpoint valid, so API.md's binding rules moved under it.
