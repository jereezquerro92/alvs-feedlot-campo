...
- **New module** `backend/apps/router/rate_abuse.py` — `evaluate_rate_abuse(user_id)` and `is_rate_blocked(user_id)`. State lives entirely in the shared Django cache ([[CACHE]] layer 2, [[adr-06-cache]]): one cache key per user for the current activity streak (`start`, `count`, `last`), one cache key per user for an active block (present ⇒ blocked, TTL = `ROUTER_RATE_BLOCK_SECONDS`).
