...
- **Outage behavior (decision 4)**: a Bedrock failure at request time (`BotoCoreError`/`ClientError`) degrades the router surface only — `503 {"detail": "router_unavailable"}` with its `IntentQuery` audit row (`choice="unavailable"`). That is **not** the `ROUTER_ENABLED=false` kill-switch shape (which is `200 { "outcome": "disabled" }` per [[API]]). The backend never crashes and the choice is never defaulted or retried ([[adr-15-chatbot-two-tier]] rule 2).
