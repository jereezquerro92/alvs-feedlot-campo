...
>
  ...
  <div style="background: var(--card); border: var(--hairline) solid var(--border);">
    <h2 class="text-sm font-semibold uppercase tracking-wide text-muted-foreground">{t("feedlot_module_group_stock_own")}</h2>
    <p class="text-xs text-muted-foreground">{t("feedlot_module_group_stock_own_hint")}</p>
      ...
      <table class="text-sm">
        <thead>
          <tr class="text-left text-xs uppercase tracking-wide text-muted-foreground">
            <th class="font-semibold">{t("feedlot_stock_col_feed")}</th>
            <th class="font-semibold">{t("feedlot_stock_col_unit")}</th>
            <th class="font-semibold text-right">{t("feedlot_stock_col_balance")}</th>
          </tr>
        </thead>
        <tbody>
          {#each stockRows as row (row.feed_type)}
            <tr style="border-top: var(--hairline) solid var(--border);">
              <td>{row.name ?? `#${row.feed_type}`}</td>
              <td class="text-muted-foreground">{row.unit ?? "—"}</td>
              <td class="text-right tabular-nums">{fmt(row.own)}</td>
            </tr>
          {/each}
        </tbody>
      </table>
  ...
  <div style="background: var(--card); border: var(--hairline) solid var(--border);">
    ...
    <p class="text-xs text-muted-foreground">{t("feedlot_module_group_stock_client_hint")}</p>
    {#if !hasClient}
        ...
        <tbody>
          {#each stockRows as row (row.feed_type)}
            <tr style="border-top: var(--hairline) solid var(--border);">
              <td>{row.name ?? `#${row.feed_type}`}</td>
              <td class="text-muted-foreground">{row.unit ?? "—"}</td>
              <td class="text-right tabular-nums">{fmt(row.client)}</td>
            </tr>
          {/each}
        </tbody>
      </table>
