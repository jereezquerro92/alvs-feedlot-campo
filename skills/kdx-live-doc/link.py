...
def _derive_slug() -> str:
    """PROJECT_SLUG when set (explicit path); otherwise this repo's own name,
    read from its git remote — never the template's reference slug. A clone
    spawned from the template has its own repository name, and that name is
    a single source of truth the linker and its callers can read alike,
    instead of a fallback describing the template it came from."""
    env = os.environ.get("PROJECT_SLUG")
    if env:
        return env
    try:
        url = subprocess.run(
...
# Naming authority: [[GLOSSARY]] ([[adr-12-glossary-and-localization]]); runtime carrier: PROJECT_SLUG ([[VARIABLES]]).
SLUG = _derive_slug()
