...
**Harness**
- [[SKILL-INVENTORY]] — the skills this template requires, vendored into `.claude/skills/` + `skills/`; ruled by [[adr-02-harness]]. Stack/AWS work goes through these skills, vault `.md` through `obsidian-markdown`, fan-out through `kdx-orchestrator`.
...
- `.claude/rules/` → `docs/adrs/` — ADRs live in the vault and load as rules through the link ([[adr-00-discipline]]).
...
- `docs/constitution/` — [[PRD]] and [[REQUIREMENTS]] live here (`docs/constitution/PRD.md`, `docs/constitution/REQUIREMENTS.md`).
- `docs/assertions/` — the assertion set the `assertion-review` skill checks against.
- `.claude/skills/` (git-tracked, harness-loaded) mirrored by `skills/`, and `docs/hooks/` mirrored to `.claude/hooks/`/`.git/hooks/` — stage 2 homes. The required skills are **vendored real copies** (self-contained, no dependence on the machine-global skill harness), inventoried in [[SKILL-INVENTORY]] and ruled by [[adr-02-harness]].
- `docs/agents/` — SSOT of the guardian subagents (`astro-drf-aws-prd`/`-adr`/`-api`) and the `orch-*`/`kwf-*` roster, with `docs/agents/souls/` holding the `kwf-*` souls; reached as `.claude/agents/` and `.agents/agents/`; the hook `dispatch_guardians.py` routes watched-file changes to them.
- `CLAUDE.md` → `AGENTS.md`.
- `tests/` — harness/structure tests (e.g. Compose layout).

## Meta-rules

- ADRs state rules, never information; content lives in the referenced doc ([[adr-00-discipline]]).
